...
%HASTA AQUI es asequible, lo siguiente no lo entiendo (pero creo que es
%cocinica.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%No me queda claro que se hace antes de esto. Es evidente que se calcula el
%nivel umbral (LevelThresh). Pero tambien se le hace un acomodado a las
%senal VAD que no se especifica en la descripcion del estandar. Senal VAD
%contiene la energia en 4 ms (32 muestras a 8Khz).

%Si VAD  NO es superior al umbral entonces el envelope es 0 (log(MAX(VAD/LevelThresh,1)))
logVAD( find( VAD<= LevelThresh))= 0;
%Si VAD es superior al umbral, entonces se divide por el Level y se le hace
%el log
VAD_greaterthan_LevelThresh= find( VAD> LevelThresh);
logVAD( VAD_greaterthan_LevelThresh)= log( VAD(VAD_greaterthan_LevelThresh)/ LevelThresh);
%LogVAD queda relleno tanto de los que superan como los que no el umbral.
